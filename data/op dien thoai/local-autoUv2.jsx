...
function createTableGlass(data) {
    ...
    var wAll = data.thongso.w, hAll = data.thongso.h;
    var tableType = data.thongso.tableType;
    var wOIn = 0, hOIn = 0;
    var FileDesign = data.thongso.LocalFileDesign;
    if (tableType === "table2") { wOIn = Math.round(data.thongso.oIn.w / 0.084667), hOIn = Math.round(data.thongso.oIn.h / 0.084667) }
    ...
    for (var i = 0; i <= arr.length - 1; i++) { // loop làm file in
        ...
        var lastName = "";
        if (tableType === "table1") {
            GLLMTableType1(arr, wAll, hAll, yPosition, xPosition, hLast, wLast, lastName, stt, i, FileDesign);
            ...
            { // lưu file khung
                ...
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Glass " + "b-" + (i + 1) + "-" + FileName + "/khung.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
            }
            {// lưu file in
                ...
                app.doAction("selectArea", "autoUv");
                app.doAction("createSPOTWithArea", "autoUv");
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Glass " + "b-" + (i + 1) + "-" + FileName + "/in.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
            ...
            { // lưu file khung
                ...
                var folder1 = Folder("~/Desktop/in an/Glass " + "b-" + (i + 1) + "-" + FileName);
                if (!folder1.exists) { folder1.create(); }
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Glass " + "b-" + (i + 1) + "-" + FileName + "/khung.jpg"), JPEGSaveOptions, true, Extension.LOWERCASE);
            }
            {// lưu file in
                app.activeDocument.rotateCanvas(180);
                // app.activeDocument.channels.getByName("Spot Color 1").remove();
    ...
    for (var i = 0; i <= arr.length - 1; i++) { // loop làm file in
        ...
        if (tableType === "table1") {
            ...
            { // lưu file khung
                app.activeDocument.layerSets.getByName("SPOT").visible = false;
                app.activeDocument.layerSets.getByName("CMYK").visible = false;
                var folder1 = Folder("~/Desktop/in an/Luminous " + "b-" + (i + 1) + "-" + FileName);
                if (!folder1.exists) { folder1.create(); }
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Luminous " + "b-" + (i + 1) + "-" + FileName + "/khung.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
            }
            {// lưu file in
                app.activeDocument.channels.getByName("Spot Color 1").remove();
                app.activeDocument.layerSets.getByName("CMYK").visible = true;
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Luminous " + "b-" + (i + 1) + "-" + FileName + "/in.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
            }
            {
                app.activeDocument.activeLayer = app.activeDocument.layerSets.getByName("CMYK");
                app.doAction("createSmarkOBJ", "autoUv");
                ...
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Luminous " + "b-" + (i + 1) + "-" + FileName + "/white.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
                app.activeDocument.close(SaveOptions.DONOTSAVECHANGES);
            }
        } else if (tableType === "table2") {
            GLLMTableType2(arr, wAll, hAll, yPosition, xPosition, hLast, wLast, lastName, stt, i, FileDesign, wOIn, hOIn);
            ...
            { // lưu file khung
                ...
                var folder1 = Folder("~/Desktop/in an/Luminous " + "b-" + (i + 1) + "-" + FileName);
                if (!folder1.exists) { folder1.create(); }
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Luminous " + "b-" + (i + 1) + "-" + FileName + "/khung.jpg"), JPEGSaveOptions, true, Extension.LOWERCASE);
            }
            {// lưu file in
                app.activeDocument.rotateCanvas(180);
                // app.activeDocument.channels.getByName("Spot Color 1").remove();
    ...
    { // tạo nhãn
        ...
        for (var i = 0; i <= arr.length - 1; i++) {
            for (var j = 0; j <= arr[i].length - 1; j++) {
                ...
                var folder1 = Folder("~/Desktop/in an/Luminous " + "b-" + (i + 1) + "-" + FileName + "/tem");
                if (!folder1.exists) { folder1.create(); }
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Luminous " + "b-" + (i + 1) + "-" + FileName + "/tem/" + arr[i][j].stt + ".jpg"), JPEGSaveOptions, true, Extension.LOWERCASE);
            }
        }
        app.activeDocument.close(SaveOptions.DONOTSAVECHANGES);
    }
            ...
            for (var ban = 0; ban <= typePosition[arr].length - 1; ban++) { // loop  1 bàn
                ...
                for (var i = 0; i <= typePosition[arr][ban].length - 1; i++) { // loop 1 hàng
                    for (var j = 0; j <= typePosition[arr][ban][i].length - 1; j++) { // lop 1 cái
                        ...
                        {
                            try {
                                app.open(File(FileDesign + "/" + typePosition[arr][ban][i][j].sku + ".tif"));
                        ...
                        {
                            try {
                                app.open(File(FileDesign + "/" + typePosition[arr][ban][i][j].sku + "_spot.tif"));
                                app.activeDocument.flipCanvas(Direction.HORIZONTAL);
...
function createTableLed(data) {
    ...
    for (var i = 0; i <= arr.length - 1; i++) { // loop làm file in
        ...
        wLast = 0;
        var lastName = "";
        LEDOneTable(arr, wAll, hAll, yPosition, xPosition, hLast, wLast, lastName, stt, i, FileDesign);
        ...
        { // lưu file khung
            ...
            app.activeDocument.layerSets.getByName("CMYK").visible = false;
            var folder1 = Folder("~/Desktop/in an/Led " + "b-" + (i + 1) + "-" + FileName);
            if (!folder1.exists) { folder1.create(); }
            app.activeDocument.saveAs(Folder("~/Desktop/in an/Led " + "b-" + (i + 1) + "-" + FileName + "/khung.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
        ...
        {// lưu file in 1
            ...
            app.activeDocument.saveAs(Folder("~/Desktop/in an/Led " + "b-" + (i + 1) + "-" + FileName + "/in 1.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
        ...
        { // lưu file in 2
            ...
            showRGBChannel();
            app.activeDocument.saveAs(Folder("~/Desktop/in an/Led " + "b-" + (i + 1) + "-" + FileName + "/in 2.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
    ...
    { // tạo nhãn
        ...
        for (var i = 0; i <= arr.length - 1; i++) {
            for (var j = 0; j <= arr[i].length - 1; j++) {
                moveTem(arr[i][j], type);
                var folder1 = Folder("~/Desktop/in an/Led " + "b-" + (i + 1) + "-" + FileName + "/tem");
                if (!folder1.exists) { folder1.create(); }
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Led " + "b-" + (i + 1) + "-" + FileName + "/tem/" + arr[i][j].stt + ".jpg"), JPEGSaveOptions, true, Extension.LOWERCASE);
...
function GLLMTableType1(arr, wAll, hAll, yPosition, xPosition, hLast, wLast, lastName, stt, i, FileDesign) {
    // for loop items
    for (var j = 0; j <= arr[i].length - 1; j++) {
...
function GLLMTableType2(arr, wAll, hAll, yPosition, xPosition, hLast, wLast, lastName, stt, i, FileDesign, wOIn, hOIn) {
    // for loop items
    for (var j = 0; j <= arr[i].length - 1; j++) {
        var wphone = arr[i][j].pixel.w;
        var hphone = arr[i][j].pixel.h;
...
function LEDOneTable(arr, wAll, hAll, yPosition, xPosition, hLast, wLast, lastName, stt, i, FileDesign) {
    ...
    for (var j = 0; j <= arr[i].length - 1; j++) {
        ...
        // lam SPOT2
        try {
            app.open(File(FileDesign + "/" + arr[i][j].sku + "flash.tif"));
        } catch (error) {
            app.documents.add(1200, 2400, 300, "aaaa");
        }
        if (app.activeDocument.width !== 1200 || app.activeDocument.height !== 2400) {
